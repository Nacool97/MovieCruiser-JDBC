...
public class FavoritesDaoSqlImpl implements FavoritesDao{
  public  long total=0;
	...
	public void addFavorites(long userId, long moviesId) {
		...
		try {
			Connection con=ConnectionHandler.getConnection();
				...
				PreparedStatement stmt=con.prepareStatement("insert into favourites (ft_us_id,ft_mo_id) values('"+userId+"','"+moviesId+"')");
				stmt.executeUpdate();
	...
	public ArrayList<Movies> getAllFavorites(long userId) throws FavoritesEmptyException {
		...
		Movies movieItem=null;
			...
			while(rs.next()){
				...
				movieItem=new Movies((long)rs.getInt(1),rs.getString(2),rs.getString(3),b,rs.getDate(5),rs.getString(6),t);
	...
	public void removeFavorites(long userId, long moviesId) {
		...
		Connection con=ConnectionHandler.getConnection();
		try {
			PreparedStatement stmt = con.prepareStatement("delete from favourites where ft_us_id='"+userId+"' and ft_mo_id='"+moviesId+"' limit 1");
			stmt.executeUpdate();
